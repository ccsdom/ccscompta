...
exports.ExtractDataOutputSchema = void 0;
exports.processDocumentContent = processDocumentContent;
exports.calculateBillableLines = calculateBillableLines;
const genkit_1 = require("genkit");
const google_genai_1 = require("@genkit-ai/google-genai");
// Re-defining the output schema for the backend processor
const TransactionSchema = genkit_1.z.object({
    date: genkit_1.z.string().describe('The date of the transaction.'),
    description: genkit_1.z.string().describe('The description or label of the transaction.'),
    amount: genkit_1.z.number().describe('The amount of the transaction. Use positive for credits, negative for debits.'),
    vendor: genkit_1.z.string().optional().describe('The identified vendor.'),
    category: genkit_1.z.string().optional().describe('The suggested accounting category.'),
    matchingDocumentId: genkit_1.z.string().optional().describe("The ID of the invoice/receipt document that matches this transaction."),
});
exports.ExtractDataOutputSchema = genkit_1.z.object({
    documentType: genkit_1.z.string().optional().describe('Detected document type (invoice, bank statement, etc.)'),
    ...
    anomalies: genkit_1.z.array(genkit_1.z.string()).optional(),
 ...
 */
async function processDocumentContent(buffer, mimeType, providedDocumentType) {
    const ai = getAi();
        ...
        const { output: recognition } = await ai.generate({
            ...
            output: {
                schema: genkit_1.z.object({
                    ...
                    isBankStatement: genkit_1.z.boolean()
                })
            }
        });
        if (recognition) {
    ...
    const { output } = await ai.generate({
        ...
        output: { schema: exports.ExtractDataOutputSchema }
    });
    if (!output) {
        throw new Error("Gemini returned no output.");
    }
    return Object.assign(Object.assign({}, output), { documentType: documentType || output.category || 'invoice' });
}
/**
