...
export default function EditInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [status, setStatus] = useState<SalesInvoice['status']>('draft');
  ...
  const loadInvoice = async () => {
    try {
      ...
      if (data) {
        ...
        setItems(data.items.map(({ id, totalHT, totalVAT, totalTTC, ...rest }) => rest));
      } else {
        toast({ variant: 'destructive', title: 'Erreur', description: 'Facture introuvable.' });
  ...
  const handleSubmit = async (newStatus?: SalesInvoice['status']) => {
    if (!customer.name) {
    ...
    try {
      ...
      await salesService.updateInvoice(id, {
        date,
        dueDate,
  ...
  const handleDownload = () => {
    if (invoice) {
        salesService.generateInvoicePDF({
            ...invoice,
            date,
            dueDate,
            customerName: customer.name,
            customerAddress: customer.address,
            items: items as any, // simplify for PDF
            totalHT: totals.ht,
            totalVAT: totals.vat,
            totalTTC: totals.ttc,
        }, userProfile);
    ...
    <div className="p-8 max-w-[1200px] mx-auto space-y-8 pb-32">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
            <Button variant="outline" onClick={handleDownload} className="rounded-xl border-white/10 bg-white/5 gap-2 font-space uppercase text-[10px] tracking-widest font-black h-11 px-6">
                <Download className="h-4 w-4" /> PDF
            </Button>
            <Button variant="outline" onClick={() => handleSubmit()} disabled={isSubmitting} className="rounded-xl border-white/10 bg-white/5 gap-2 font-space uppercase text-[10px] tracking-widest font-black h-11 px-6">
      ...
      <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-black font-space tracking-tight">Facture {invoice?.invoiceNumber}</h1>
          <div className="flex items-center gap-2">
            <div className={`h-2 w-2 rounded-full ${status === 'paid' ? 'bg-emerald-500' : status === 'sent' ? 'bg-blue-500' : 'bg-slate-400'}`} />
            <span className="text-[10px] font-black uppercase tracking-widest opacity-50">{status}</span>
          </div>
      </div>
