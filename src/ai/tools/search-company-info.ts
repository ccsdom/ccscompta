...
const searchBySiret = async (siret: string, token: string) => {
    ...
    // The API doesn't provide a direct legal representative, phone or email. This would need other data sources.
    return {
...
const searchByName = async (name: string, token: string) => {
     const response = await fetch(`https://api.insee.fr/entreprises/sirene/V3.11/siret?q=denominationUniteLegale:"${encodeURIComponent(name)}" AND periode.etatAdministratifEtablissement:A`, {
        headers: { Authorization: `Bearer ${token}` },
    });
     if (!response.ok) return null;
     const data = await response.json();
